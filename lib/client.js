  ...
  factory: (require) => {
    ...
    async function autoCreateAndStart(workspaces, sessions, fallbackRoot) {
        ...
        try {
            ...
            if (!result.ok)
                throw new Error(result.error?.message ?? 'session create failed');
    ...
    function WorkspaceState(props) {
        ...
        }, React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8 } }, React.createElement('span', { role: 'status', style: { fontSize: 13, opacity: selected ? 1 : 0.75 } }, selected ? `工作区：${selectedTitle || selectedId}` : '默认工作区'), selected && React.createElement('button', {
            type: 'button',
            'aria-label': '取消当前工作区',
            title: '取消当前工作区',
            onClick: clear,
        ...
        }, '×')), status?.phase === 'busy' && React.createElement('span', { role: 'status', style: { fontSize: 12, opacity: 0.7 } }, '正在创建临时任务…'), status?.phase === 'error' && React.createElement('div', { role: 'alert', style: { fontSize: 12, color: '#e5484d' } }, `临时任务创建失败：${status.message}`));
    }
    ...
    function apply(ctx, config) {
        ...
        const sidebarOccupant = (owner) => React.createElement(SidebarFlow, { owner, pick, create, root: fallbackRoot });
        const injected = () => ({});
