...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function isUngroupedGroupElement(group) {
        if (group === null || typeof group !== 'object' || group.props === undefined)
            return false;
        if (group.key === '')
            return true;
        const children = group.props.children;
        const first = Array.isArray(children) ? children[0] : children;
        const groupProp = first?.props?.group;
        return groupProp !== undefined && groupProp.workspaceId === undefined;
    }
        ...
        if (Array.isArray(children)) {
            ...
            const next = children.map((child) => {
                ...
                const groups = child.map((group) => {
                    if (isUngroupedGroupElement(group) && group.props?.['data-timestamp-ungrouped'] === undefined) {
                        groupsChanged = true;
