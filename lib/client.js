...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     */
    async function cleanupUnusedTemporaryTasks(workspaces, sessions) {
        ...
        for (const id of sessionState.ids ?? []) {
            ...
            if (!summary?.cwd || !summary.cwd.startsWith(normalized + '/') && !summary.cwd.startsWith(normalized + ''))
                continue;
            ...
            for (const group of Array.from(tree.children)) {
                ...
                if (actions !== null && !actions.querySelector('[data-timestamp-temp-cleanup]')) {
                    ...
                    clean.addEventListener('click', () => {
                        ...
                        const ss = runtime?.sessions;
                        if (!ws || !ss)
                            return;
                        if (!window.confirm('清理所有未使用的临时任务？（将删除对应文件夹并归档空白会话）'))
                            return;
                        cleanupUnusedTemporaryTasks(ws, ss).then((count) => {
                            if (count > 0)
                                window.alert(`已清理 ${count} 个未使用的临时任务`);
                            else
                                window.alert('没有可清理的未使用临时任务');
                        }).catch(() => { });
                    });
