...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function resolveWorkspacesFacade(_ctx) {
        ...
        const createDirectory = (root, name) => {
            const { ws, ui } = live();
            if (typeof ws?.createDirectory === 'function')
                return ws.createDirectory(root, name);
            if (typeof ui?.createDirectory === 'function')
                return ui.createDirectory(root, name);
            throw new Error('workspaces.createDirectory 不可用（宿主版本不兼容）');
        };
        const pickDirectory = () => {
            const { ws, ui } = live();
            if (typeof ws?.pickDirectory === 'function')
        ...
        const archiveSession = (sessionId) => {
            ...
            if (typeof ws?.archiveSession === 'function')
                return ws.archiveSession(sessionId);
            if (typeof ui?.archiveSession === 'function')
                return ui.archiveSession(sessionId);
            throw new Error('workspaces.archiveSession 不可用（宿主版本不兼容）');
        };
        const facade = { createDirectory, pickDirectory, archiveSession };
