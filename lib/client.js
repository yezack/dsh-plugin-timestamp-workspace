...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     */
    function injectBatchArchiveMenu(menu, label, onPick) {
        const viewport = menu.querySelector('[role="presentation"]');
        if (viewport === null)
            return;
        if (viewport.querySelector('[' + BATCH_ARCHIVE_ITEM_ATTR + ']') !== null)
            return;
    ...
    function BatchArchiveDialog(props) {
        ...
        const archived = new Set(workspaceState.archivedSessionIds ?? []);
        const workspace = (workspaceState.items ?? []).find((w) => w.workspaceId === workspaceId);
    ...
    function WorkspaceBrowserOrderWrapper(props) {
        ...
        useLayout(() => {
            ...
            for (const candidate of Array.from(tree.querySelectorAll('[role="treeitem"]'))) {
                const el = candidate;
                if (el.querySelector('button[aria-label*="的操作"]') === null)
                    continue;
                if (el.querySelector('button[aria-label*="新建会话"]') === null)
                    continue;
                workspaceRows.push(el);
            }
            workspaceRows.forEach((row, index) => {
                const ellipsis = row.querySelector('button[aria-label*="的操作"]');
                if (ellipsis === null || ellipsis.hasAttribute(BATCH_ARCHIVE_ATTR))
                    return;
                ...
                let workspaceId = titleMatches.length === 1 ? titleMatches[0].workspaceId : undefined;
                if (workspaceId === undefined)
                    workspaceId = items[index]?.workspaceId;
                if (workspaceId === undefined || rowLabel === '')
                    return;
        ...
        return React.createElement('div', { ref: containerRef, style: { display: 'contents' } }, React.createElement(nativeWorkspaceBrowserOriginal, props), batchArchive === null ? null : React.createElement(BatchArchiveDialog, {
