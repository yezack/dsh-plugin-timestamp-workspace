...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    exports.BatchArchiveDialog = BatchArchiveDialog;
    exports.apply = apply;
     ...
     */
    function resolveWorkspacesFacade(_ctx) {
     ...
     */
    async function createDirectoryViaHost(root, name) {
        try {
    ...
    function startTemporaryTask() {
        if (runtime === null)
            return;
        void autoCreateAndStart(workspacesService, runtime.sessions);
    }
    function TimestampSettingsSection(props) {
        ...
        return React.createElement('div', { style: { padding: '4px 0', display: 'flex', flexDirection: 'column', gap: 12 } }, React.createElement('p', { style: { margin: 0, fontSize: 13, opacity: 0.8 } }, '开启临时会话时，将在此根目录下自动创建 YYYYMMDDHHmmss 命名的工作区文件夹。保存后立即生效，优先于 cordis.patch.yml 里的 rootDirectory 配置。'), loading
            ...
            : React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 8 } }, React.createElement('label', { style: { fontSize: 13, display: 'flex', flexDirection: 'column', gap: 4 } }, '根目录（rootDirectory）', React.createElement('input', {
            ...
            })), React.createElement('div', { style: { display: 'flex', gap: 8 } }, React.createElement('button', {
                ...
                onClick: pick,
            }, busy ? '处理中…' : '选择目录…'), React.createElement('button', { disabled: busy, onClick: save }, '保存')), error && React.createElement('div', { role: 'alert', style: { fontSize: 13, color: '#e5484d' } }, error), saved && React.createElement('div', { style: { fontSize: 13, color: '#30a46c' } }, '已保存')), props.close && React.createElement('button', { onClick: props.close }, '完成'));
    }
    /**
    ...
    function getPrimitives() {
        ...
        const req = globalThis.require ?? (typeof require === 'function' ? require : undefined);
    ...
    function HeroTempButton(props) {
        ...
        useLayout(() => {
            ...
            if (myButton !== null && all[0] !== myButton)
                el.style.display = 'none';
        });
        return React.createElement('div', { ref: rootRef, style: { display: 'inline-flex', alignItems: 'center', minWidth: 0 } }, React.createElement('button', {
            type: 'button',
    ...
    function WorkspacePickerWrapper(props) {
        ...
        const original = props.__timestampPickerOriginal;
        return React.createElement(React.Fragment, null, React.createElement(HeroTempButton, { status, onStart: startTemporaryTask }), original === undefined ? null : React.createElement(original, props));
    }
    // Wrap EVERY live entry of the hero picker slot: the slot renderer elects
    // per-cell winners, and with several registrations (a skin, a hot-reload
    // re-registration, a second window) the rendered winner may be any of them.
    ...
    const wrappedPickerEntries = new WeakSet();
    let pickerSlotsRef = null;
    let pickerRetryTimer = null;
    ...
    function installWorkspacePickerOverride(slots) {
        ...
        const candidates = entries.filter((candidate) => typeof candidate.component === 'function');
        let wrapped = 0;
        for (const candidate of candidates) {
            if (wrappedPickerEntries.has(candidate))
                continue;
            wrappedPickerEntries.add(candidate);
            if (nativeWorkspacePickerOriginal === null) {
                nativeWorkspacePickerEntry = candidate;
                nativeWorkspacePickerOriginal = candidate.component;
            }
            const original = candidate.component;
            candidate.component = (props) => React.createElement(WorkspacePickerWrapper, { ...props, __timestampPickerOriginal: original });
            wrapped += 1;
        }
        if (!pickerDiagnosed) {
     ...
     */
    function BatchArchiveDialog(props) {
        ...
        const workspace = workspaceId === undefined
            ? undefined
            : (workspaceState.items ?? []).find((w) => w.workspaceId === workspaceId);
        const titleOf = (summary, id) => summary?.blank === true ? '新会话' : (summary?.displayTitle ?? summary?.title ?? id);
        let rows;
        if (workspace !== undefined) {
            rows = (workspace.sessionIds ?? [])
                .filter((id) => !archived.has(id))
                .map((id) => ({ id, title: titleOf(sessionState.byId?.[id], id) }));
        }
        else {
        ...
        const selectBody = React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 8, minWidth: 340 } }, React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 12, color: 'var(--dsw-alias-label-secondary)' } }, React.createElement('span', null, `已选 ${chosen} / ${count}`), React.createElement('div', { style: { display: 'flex', gap: 10 } }, React.createElement('button', { type: 'button', disabled: busy, onClick: () => setAll(true), style: linkStyle }, '全选'), React.createElement('button', { type: 'button', disabled: busy, onClick: () => setAll(false), style: linkStyle }, '清空'))), count === 0
            ? React.createElement('div', { style: { padding: '12px 8px', fontSize: 13, color: 'var(--dsw-alias-label-secondary)' } }, '该工作区没有可归档的会话')
            : React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 2, maxHeight: 300, overflowY: 'auto' } }, rows.map((row) => React.createElement('button', {
                key: row.id, type: 'button', disabled: busy, onClick: () => toggle(row.id),
                style: rowStyle(selected.has(row.id)),
            }, React.createElement('span', { style: { flex: 1, minWidth: 0, textAlign: 'left', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, row.title), React.createElement('span', { style: checkStyle(selected.has(row.id)), dangerouslySetInnerHTML: { __html: CHECK_ICON_SVG } })))), error === null ? null : React.createElement('div', { role: 'alert', style: { fontSize: 13, color: 'var(--dsw-alias-state-error-primary)' } }, error));
        const chosenRows = rows.filter((row) => selected.has(row.id));
        const confirmBody = React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 10, minWidth: 340 } }, React.createElement('p', { style: { margin: 0, fontSize: 13, lineHeight: '20px' } }, `确定归档选中的 ${chosen} 个会话吗？归档后它们会从侧边栏隐藏，会话日志保留，之后仍可恢复。`), React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 4, padding: '8px 10px', borderRadius: 8, border: '1px solid var(--dsw-alias-border-l3)', fontSize: 13, lineHeight: '20px', maxHeight: 200, overflowY: 'auto' } }, chosenRows.map((row) => React.createElement('div', { key: row.id, style: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, row.title))), error === null ? null : React.createElement('div', { role: 'alert', style: { fontSize: 13, color: 'var(--dsw-alias-state-error-primary)' } }, error));
        const selectFooter = React.createElement(React.Fragment, null, React.createElement(Button, { variant: 'outline', disabled: busy, onClick: onClose }, '取消'), React.createElement(Button, { variant: 'primary', disabled: busy || chosen === 0, onClick: () => setStep('confirm') }, chosen > 0 ? `归档 ${chosen} 个` : '归档'));
        const confirmFooter = React.createElement(React.Fragment, null, React.createElement(Button, { variant: 'outline', disabled: busy, onClick: () => setStep('select') }, '返回'), React.createElement(Button, { variant: 'primary', disabled: busy, onClick: () => void runArchive() }, busy ? '归档中…' : '确认归档'));
        if (Modal === undefined || Button === undefined) {
            ...
            return React.createElement('div', { style: { position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center' } }, React.createElement('div', { role: 'dialog', style: { background: 'var(--dsw-alias-surface-bg-l1)', borderRadius: 12, padding: 16, minWidth: 340, maxWidth: 480 } }, React.createElement('strong', null, `批量归档 · ${label}`), step === 'select' ? selectBody : confirmBody, React.createElement('div', { style: { display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 } }, React.createElement('button', { type: 'button', disabled: busy, onClick: step === 'select' ? onClose : () => setStep('select') }, step === 'select' ? '取消' : '返回'), React.createElement('button', { type: 'button', disabled: busy || chosen === 0, onClick: step === 'select' ? () => setStep('confirm') : () => void runArchive() }, step === 'select' ? (chosen > 0 ? `归档 ${chosen} 个` : '归档') : (busy ? '归档中…' : '确认归档')))));
        }
    ...
    function WorkspaceBrowserOrderWrapper(props) {
        ...
        const useLayout = React.useLayoutEffect ?? React.useEffect;
        const [batchArchive, setBatchArchive] = React.useState(null);
        const [ungroupedMenu, setUngroupedMenu] = React.useState(null);
        ...
        if (sessionState !== undefined && workspaceState !== undefined) {
            ...
            ungroupedCount = (sessionState.ids ?? []).filter((id) => {
                const summary = sessionState.byId?.[id];
                if (summary === undefined || registered.has(id) || archived.has(id))
                    return false;
                if (summary.origin === 'subagent')
                    return false;
                if (summary.blank === true && id !== sessionState.current)
        ...
        useLayout(() => {
            ...
            for (const group of Array.from(tree.children)) {
                ...
                if (plus !== null) {
                    let ellipsis = row.querySelector('button[data-timestamp-ungrouped-ellipsis]');
                    if (ellipsis === null) {
        ...
        return React.createElement('div', { ref: containerRef, style: { display: 'contents' } }, React.createElement(nativeWorkspaceBrowserOriginal, props), ungroupedMenu !== null && MenuComp !== undefined
            ? React.createElement(MenuComp, {
                ...
                onSelect: (id) => {
                    ...
                    if (id === 'batch-archive')
                        setBatchArchive({ workspaceId: undefined, label: '未分组' });
                },
            ...
            : null, batchArchive === null ? null : React.createElement(BatchArchiveDialog, {
    ...
    function apply(ctx, config) {
        ...
        workspacesService = resolveWorkspacesFacade(ctx);
        const workspaces = workspacesService;
