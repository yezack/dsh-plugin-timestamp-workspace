...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    async function autoCreateAndStart(workspaces, fallbackRoot) {
        if (autoCreating || !workspaces)
            return;
        autoCreating = true;
        try {
    ...
    function apply(ctx, config) {
        ...
        if (workspaces && typeof workspaces.startSession === 'function' && originalStartSession === null) {
            ...
            workspaces.startSession = (workspaceId) => {
                if (workspaceId === undefined)
                    return autoCreateAndStart(workspaces, config.rootDirectory);
        ...
        const heroOccupant = (owner) => React.createElement(HeroFlow, { owner, pick, create, root: config.rootDirectory, workspaces });
        const sidebarOccupant = (owner) => React.createElement(SidebarFlow, { owner, pick, create, root: config.rootDirectory });
        const injected = () => ({});
