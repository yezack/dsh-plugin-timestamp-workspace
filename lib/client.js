  ...
  factory: (require) => {
    ...
    function installNativeComposerOverride(slots, sessions, workspaces) {
        ...
        entry.component = (props) => {
            ...
            const clear = () => {
                ...
                if (current !== undefined && typeof ws?.sessions?.clear === 'function')
                    ws.sessions.clear();
            };
            const accessory = temporary
                ? React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8 } }, props.accessory, React.createElement('button', {
                    type: 'button',
                    'aria-label': '清除临时对话',
                    title: '清除临时对话',
                    onClick: clear,
    ...
    function reorderTemporaryGroupFirst(node) {
        ...
        if (Array.isArray(node))
            return node.map(reorderTemporaryGroupFirst);
