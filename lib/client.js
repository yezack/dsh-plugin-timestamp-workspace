...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     */
    function moveUngroupedFirst(node) {
        if (node === null || node === undefined || typeof node !== 'object')
            return node;
        if (Array.isArray(node))
            return node.map(moveUngroupedFirst);
        ...
        if (props.role === 'tree' && Array.isArray(children)) {
            const next = children.map((child) => {
                if (!Array.isArray(child))
                    return child;
                const idx = child.findIndex((group) => containsText(group, '未分组') || containsText(group, 'Ungrouped'));
                if (idx <= 0)
                    return child;
                return [child[idx], ...child.filter((_, i) => i !== idx)];
            });
    ...
    function installWorkspaceBrowserOverride(slots) {
        ...
        entry.component = (props) => {
            ...
            try {
                return moveUngroupedFirst(element);
            }
            catch {
                return element;
            }
