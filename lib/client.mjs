...
function makeSelectTemporaryTask(sessions, workspaces) {
	return ({ session }) => {
		const sessionId = session?.sessionId ?? session?.session?.sessionId;
		if (!sessionId) return null;
		if (!(((sessions?.list?.getSnapshot?.())?.byId?.[sessionId])?.cwd ?? session?.cwd ?? session?.session?.cwd)) return null;
		return (workspaces?.list?.getSnapshot?.()?.items ?? []).some((item) => item.sessionIds?.includes(sessionId)) ? null : session;
	};
}
function TemporaryTaskComposer(props) {
	const input = props.useInput?.((state) => state);
	const actions = props.inputActions;
	const draft = input?.draft ?? "";
	const busy = input?.phase === "adjudicating" || input?.phase === "submitting";
	const disabled = actions === void 0 || input === void 0 || busy;
	const submit = () => {
