...
function WorkspaceBrowserOrderWrapper(props) {
	...
	(React.useLayoutEffect ?? React.useEffect)(() => {
		...
		for (const group of Array.from(tree.children)) {
			...
			if (!(row.getAttribute("draggable") === "false" || row.querySelector("[aria-label*=\"的操作\"]") === null)) continue;
			group.setAttribute("data-timestamp-ungrouped", "");
			const title = row.querySelector(".YDXeBa_title") ?? Array.from(row.querySelectorAll("span")).find((s) => {
				const text = (s.textContent ?? "").trim();
				return text === "未分组" || text === "Ungrouped";
			});
			if (title !== void 0 && title !== null && !(title.textContent ?? "").startsWith("⚡")) title.textContent = "⚡ 临时 · " + title.textContent;
...
function installUngroupedStyle() {
	...
	style.textContent = "[data-timestamp-ungrouped]{font-style:italic;border:1px dashed var(--dsw-alias-border-l3);border-radius:12px;margin:4px 8px;padding:2px}";
