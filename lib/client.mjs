...
function resolveWorkspacesFacade(_ctx) {
	...
	Object.defineProperty(facade, "createDirectory", { get: () => typeof ws()?.createDirectory === "function" ? ws().createDirectory.bind(ws()) : void 0 });
	Object.defineProperty(facade, "pickDirectory", { get: () => typeof ws()?.pickDirectory === "function" ? ws().pickDirectory.bind(ws()) : void 0 });
	Object.defineProperty(facade, "archiveSession", { get: () => typeof ws()?.archiveSession === "function" ? ws().archiveSession.bind(ws()) : void 0 });
	return facade;
}
/**
...
async function autoCreateAndStart(workspaces, sessions) {
	...
	try {
		...
		const path = await withTimeout(createTimestampWorkspace(createDirectoryViaHost, trimmed), 2e4, "createDirectory");
...
function startTemporaryTask() {
	...
	autoCreateAndStart(workspacesService, runtime.sessions);
}
function TimestampSettingsSection(props) {
	...
	return React.createElement("div", { style: {
		...
		title: typeof workspacesService?.pickDirectory === "function" ? void 0 : "当前宿主不支持目录选择器，请手动输入路径",
		onClick: pick
...
function getPrimitives() {
	...
	const req = globalThis.require ?? (typeof __require === "function" ? __require : void 0);
...
const wrappedPickerEntries = /* @__PURE__ */ new WeakSet();
...
function installWorkspacePickerOverride(slots) {
	...
	for (const candidate of candidates) {
		...
		if (nativeWorkspacePickerOriginal === null) nativeWorkspacePickerOriginal = candidate.component;
		const original = candidate.component;
		candidate.component = (props) => React.createElement(WorkspacePickerWrapper, {
			...props,
			__timestampPickerOriginal: original
		});
		wrapped += 1;
	}
	if (!pickerDiagnosed) {
	...
	} else if (wrapped > 0) console.log("[timestamp-workspace] hero picker late wrap: entries =", entries.length, "| newly wrapped =", wrapped);
...
function armPickerRetry(ctx) {
	...
	const tick = () => {
		...
		if (pickerSlotsRef === null) return;
		if (!installWorkspacePickerOverride(pickerSlotsRef) && pickerRetryCount < PICKER_RETRY_MAX) {
			pickerRetryCount += 1;
...
function BatchArchiveDialog(props) {
	...
	const sessionState = (runtime?.sessions ?? {})?.list?.getSnapshot?.() ?? { byId: {} };
	const workspaceState = workspaces?.list?.getSnapshot?.() ?? {
		items: [],
		archivedSessionIds: []
	};
	const archived = new Set(workspaceState.archivedSessionIds ?? []);
	const workspace = workspaceId === void 0 ? void 0 : (workspaceState.items ?? []).find((w) => w.workspaceId === workspaceId);
	const titleOf = (summary, id) => summary?.blank === true ? "新会话" : summary?.displayTitle ?? summary?.title ?? id;
	let rows;
	if (workspace !== void 0) rows = (workspace.sessionIds ?? []).filter((id) => !archived.has(id)).map((id) => ({
		id,
		title: titleOf(sessionState.byId?.[id], id)
	}));
	else {
		const registered = new Set((workspaceState.items ?? []).flatMap((w) => w.sessionIds ?? []));
		rows = (sessionState.ids ?? []).filter((id) => {
			const summary = sessionState.byId?.[id];
	...
	const setAll = (on) => {
		setSelected(on ? new Set(rows.map((row) => row.id)) : /* @__PURE__ */ new Set());
	};
	...
	const selectBody = React.createElement("div", { style: {
		display: "flex",
		flexDirection: "column",
		gap: 8,
		minWidth: 340
	...
	} }, React.createElement("span", null, `已选 ${chosen} / ${count}`), React.createElement("div", { style: {
		display: "flex",
		gap: 10
	} }, React.createElement("button", {
		type: "button",
		...
		style: linkStyle
	}, "清空"))), count === 0 ? React.createElement("div", { style: {
		padding: "12px 8px",
		fontSize: 13,
		color: "var(--dsw-alias-label-secondary)"
	} }, "该工作区没有可归档的会话") : React.createElement("div", { style: {
		display: "flex",
		flexDirection: "column",
		gap: 2,
		maxHeight: 300,
		...
		style: checkStyle(selected.has(row.id)),
		dangerouslySetInnerHTML: { __html: CHECK_ICON_SVG }
	...
	const confirmBody = React.createElement("div", { style: {
		display: "flex",
		flexDirection: "column",
		gap: 10,
		minWidth: 340
		...
		fontSize: 13,
		lineHeight: "20px"
	} }, `确定归档选中的 ${chosen} 个会话吗？归档后它们会从侧边栏隐藏，会话日志保留，之后仍可恢复。`), React.createElement("div", { style: {
		display: "flex",
		flexDirection: "column",
		gap: 4,
		padding: "8px 10px",
		...
		maxHeight: 200,
		overflowY: "auto"
	} }, chosenRows.map((row) => React.createElement("div", {
		key: row.id,
		style: {
			overflow: "hidden",
			textOverflow: "ellipsis",
	...
	}, row.title))), error === null ? null : React.createElement("div", {
		role: "alert",
		style: {
			fontSize: 13,
			color: "var(--dsw-alias-state-error-primary)"
	...
	const selectFooter = React.createElement(React.Fragment, null, React.createElement(Button, {
		variant: "outline",
		disabled: busy,
		onClick: onClose
	...
	}, chosen > 0 ? `归档 ${chosen} 个` : "归档"));
	const confirmFooter = React.createElement(React.Fragment, null, React.createElement(Button, {
		variant: "outline",
		disabled: busy,
		onClick: () => setStep("select")
	}, "返回"), React.createElement(Button, {
		variant: "primary",
		disabled: busy,
		onClick: () => void runArchive()
	}, busy ? "归档中…" : "确认归档"));
	if (Modal === void 0 || Button === void 0) return React.createElement("div", { style: {
		position: "fixed",
		inset: 0,
		zIndex: 9999,
		...
		role: "dialog",
		style: {
			background: "var(--dsw-alias-surface-bg-l1)",
			borderRadius: 12,
			padding: 16,
			minWidth: 340,
			maxWidth: 480
		}
	}, React.createElement("strong", null, `批量归档 · ${label}`), step === "select" ? selectBody : confirmBody, React.createElement("div", { style: {
		display: "flex",
	...
	}, step === "select" ? "取消" : "返回"), React.createElement("button", {
		type: "button",
		disabled: busy || chosen === 0,
		onClick: step === "select" ? () => setStep("confirm") : () => void runArchive()
...
function WorkspaceBrowserOrderWrapper(props) {
	...
	if (sessionState !== void 0 && workspaceState !== void 0) {
		...
		ungroupedCount = (sessionState.ids ?? []).filter((id) => {
			const summary = sessionState.byId?.[id];
			if (summary === void 0 || registered.has(id) || archived.has(id)) return false;
			if (summary.origin === "subagent") return false;
			if (summary.blank === true && id !== sessionState.current) return false;
			return true;
		}).length;
	...
	return React.createElement("div", {
	...
	}, React.createElement(nativeWorkspaceBrowserOriginal, props), ungroupedMenu !== null && MenuComp !== void 0 ? React.createElement(MenuComp, {
		open: true,
		anchor: null,
		items: [{
			id: "batch-archive",
			label: "批量归档",
			icon: React.createElement("span", { dangerouslySetInnerHTML: { __html: ARCHIVE_ICON_SVG } })
		}],
		onSelect: (id) => {
			setUngroupedMenu(null);
			if (id === "batch-archive") setBatchArchive({
				workspaceId: void 0,
				label: "未分组"
			});
		},
		...
		getAnchorRect: () => {
			...
			return btn !== null && btn !== void 0 ? btn.getBoundingClientRect() : null;
		}
...
//#endregion
export { BatchArchiveDialog, apply, createTimestampWorkspace, formatTimestamp, inject, name };