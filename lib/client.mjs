...
*/
/**
* The ungrouped groupSection renders ProjectRowItem first, whose group prop
* has workspaceId === undefined — the reliable structural signal (a real
* workspace named "未分组" keeps its uuid). key === "" is kept as a fallback.
*/
function isUngroupedGroupElement(group) {
	if (group === null || typeof group !== "object" || group.props === void 0) return false;
	if (group.key === "") return true;
	const children = group.props.children;
	const groupProp = (Array.isArray(children) ? children[0] : children)?.props?.group;
	return groupProp !== void 0 && groupProp.workspaceId === void 0;
}
function markUngrouped(node) {
	if (node === null || node === void 0 || typeof node !== "object") return node;
	if (Array.isArray(node)) {
		let changed = false;
		const mapped = node.map((child) => {
			const next = markUngrouped(child);
	...
	if (Array.isArray(children)) {
		...
		const next = children.map((child) => {
			...
			const groups = child.map((group) => {
				if (isUngroupedGroupElement(group) && group.props?.["data-timestamp-ungrouped"] === void 0) {
					groupsChanged = true;
					return React.cloneElement(group, {
						...group.props,
						"data-timestamp-ungrouped": "true"
					});
				}
				return group;
			});
			if (groupsChanged) changed = true;
			return groupsChanged ? groups : child;
		});
...
function WorkspaceBrowserOrderWrapper(props) {
	...
	(React.useLayoutEffect ?? React.useEffect)(() => {
		...
		if (tree === null) return;
		const group = tree.querySelector("[data-timestamp-ungrouped]");
		if (group !== null && tree.firstElementChild !== group) tree.insertBefore(group, tree.firstElementChild);
	});
	return React.createElement("div", {
		ref: containerRef,
...
function installWorkspaceBrowserOverride(slots) {
	...
	entry.component = (props) => {
		let element;
		try {
			element = markUngrouped(React.createElement(nativeWorkspaceBrowserOriginal, props));
		} catch {
			element = React.createElement(nativeWorkspaceBrowserOriginal, props);
		}
		return React.createElement(WorkspaceBrowserOrderWrapper, {
			...props,
			__element: element
		});
	};
