...
function BatchArchiveDialog(props) {
	...
	const archived = new Set(workspaceState.archivedSessionIds ?? []);
	const rows = ((workspaceState.items ?? []).find((w) => w.workspaceId === workspaceId)?.sessionIds ?? []).filter((id) => !archived.has(id)).map((id) => {
		const summary = sessionState.byId?.[id];
		return {
			id,
			title: summary?.blank === true ? "新会话" : summary?.displayTitle ?? summary?.title ?? id
		};
	});
...
function WorkspaceBrowserOrderWrapper(props) {
	...
	useLayout(() => {
		...
		workspaceRows.forEach((row, index) => {
			...
			const rowLabel = Array.from(row.querySelectorAll("span")).filter((s) => (s.textContent ?? "").trim() !== "").pop()?.textContent?.trim() ?? "";
			const titleMatches = items.filter((w) => w.title === rowLabel);
			let workspaceId = titleMatches.length === 1 ? titleMatches[0].workspaceId : void 0;
			if (workspaceId === void 0) workspaceId = items[index]?.workspaceId;
			if (workspaceId === void 0 || rowLabel === "") return;
	...
	}, React.createElement(nativeWorkspaceBrowserOriginal, props), batchArchive === null ? null : React.createElement(BatchArchiveDialog, {
