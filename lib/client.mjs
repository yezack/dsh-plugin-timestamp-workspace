...
function FlowDialogHost(props) {
	...
	const run = async (operation) => {
		...
		try {
			...
			if (path) {
				console.log("[timestamp-workspace] flow picked path:", path);
				owner.onPicked(path);
			} else {
				console.log("[timestamp-workspace] flow cancelled");
				owner.onCancel();
			}
		} catch (reason) {
			const message = reason instanceof Error ? reason.message : String(reason);
	...
	return FlowDialog({
		...
		onPick: () => run(pick),
		onCreate: () => run(async () => {
			return createTimestampWorkspace(create, await resolveRoot(props.root));
		}),
