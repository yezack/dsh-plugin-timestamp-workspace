...
function installNativeComposerOverride(slots, sessions, workspaces) {
	...
	entry.component = (props) => {
		...
		const clear = () => {
			const ws = workspaces;
			if (sessions?.list?.getSnapshot?.()?.current !== void 0 && typeof ws?.sessions?.clear === "function") ws.sessions.clear();
		};
		const accessory = temporary ? React.createElement("div", { style: {
			display: "flex",
			alignItems: "center",
			gap: 8
		} }, props.accessory, React.createElement("button", {
			type: "button",
			"aria-label": "清除临时对话",
			title: "清除临时对话",
			onClick: clear,
...
function reorderTemporaryGroupFirst(node) {
	...
	if (props.role === "tree" && Array.isArray(props.children)) {
		...
		const idx = groups.findIndex((child) => containsText(child, "临时对话"));
		if (idx > 0) {
	...
	if (props.children !== void 0) return React.createElement(node.type, props, reorderTemporaryGroupFirst(props.children));
	return node;
}
...
function installWorkspaceBrowserOverride(slots) {
	...
	entry.component = (props) => {
		const element = React.createElement(nativeWorkspaceBrowserOriginal, {
			...props,
			t: (key, args) => key === "group.ungrouped" ? "临时对话" : props.t(key, args)
		});
		try {
			return reorderTemporaryGroupFirst(element);
		} catch {
			return element;
		}
	};
...
function apply(ctx, config) {
	...
	if (workspaces && typeof workspaces.startSession === "function" && originalStartSession === null) {
		...
		workspaces.startSession = (_workspaceId) => {
