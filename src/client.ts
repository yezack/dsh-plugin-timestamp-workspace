...
function mountTemporaryConversationSection(sessions: any, workspaces: any): () => void {
...
`
  ...
  const render = () => {
    ...
    const registered = new Set((workspaceState?.items ?? []).flatMap((item: any) => item.sessionIds ?? []))
...
export function apply(ctx: ClientContext, config?: Config): void {
  ...
  const effect = (ctx as any).effect
