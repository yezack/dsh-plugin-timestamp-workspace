...
function HeroFlow(props: { owner: DirectoryFlowOwnerProps; pick: () => Promise<string | null>; create: (root: string, name: string) => Promise<string>; root: string; workspaces: WorkspaceService | undefined }) {
  const { owner, workspaces, create } = props
  if (owner.open) return React.createElement(FlowDialogHost, { owner, pick: props.pick, create: props.create, root: props.root, workspaces })
  // Blank state: offer starting a temporary conversation explicitly. The
  // timestamp folder is only created at this point — never when New Session
  // is clicked (host creates the cwd directory eagerly, so folders must not
  // be allocated before the user actually begins a conversation).
  return React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, margin: '8px 0' } },
    React.createElement('button', {
      type: 'button',
      onClick: () => autoCreateAndStart(props.workspaces ?? (runtime as any)?.workspaces, (runtime as any)?.sessions, props.root),
      style: { padding: '6px 12px', borderRadius: 8, border: '1px solid var(--dsw-alias-border-l2)', background: 'transparent', color: 'inherit', cursor: 'pointer', fontSize: 13 },
    }, '⚡ 开始临时对话'))
}
...
export function apply(ctx: ClientContext, config?: Config): void {
  ...
  if (workspaces && typeof workspaces.startSession === 'function' && originalStartSession === null) {
    ...
    // New Session never allocates a timestamp folder: it clears to the blank
    // view (host default) and the user starts a temporary conversation from
    // the hero button, which is when the folder is actually created. Explicit
    // targets (per-workspace New Session) still run the host logic.
